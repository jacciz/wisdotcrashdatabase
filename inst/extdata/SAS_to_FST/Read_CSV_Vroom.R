...
save_crash_to_fst <-
  function(filename,
    ...
    if (grepl("crash|vehicle|person", data_type)) { # Make sure not to select narrative
      ...
      } else if (data_year <= 16 & data_year >= 80) { # 1980 - 2016
        openfile$ACCDDATE <- mdy(openfile$ACCDDATE)
      # } else if (data_year >= 80 & data_year <= 94) {
...
x <- mapply(save_crash_to_fst, filename = myfiles) # batch apply function to all myfiles


old14 <- vroom::vroom("C:/CSV/csv_from_sas/from_sas_csv/14person.csv", delim = ",")
old17 <- vroom::vroom("C:/CSV/csv_from_sas/from_sas_csv/17person.csv", delim = ",", col_select = c(starts_with("CRSH"),"CNTYCODE"))
