...
library(dplyr)
import_db_data(filepath = "C:/CSV/csv_from_sas/fst/", db_type = "crash", years_old = c("15", "16"), years = c("17","18"),  columns = c("DRVRPC")) %>% head()

```

## Crash flags
Flags can be found:
+ deer_flag (old & new db) 
+ distracted_flag (new db) 
+ impaireddriver_flag (new db) 
+ speed_flag (old & new db) 
+ teendriver_flag (new db) 
+ olderdriver_flag (new db)
+ get_seatbelt_flag_by_unit
